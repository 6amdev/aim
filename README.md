...
**ครบวงจรอัตโนมัติ:** `python -m aim do "<งาน>"` — route → โหลด SKILL.md ของ pick #1 → LLM ลงมือทำ → คืนผลงานจริง (skill เท่านั้น; tool/agent/rag บอกให้ไปเรียก harness จริง)

**วัดคุณภาพ router:** `python -m aim eval` (hit@k + MRR บน [data/evals.json](data/evals.json) 30 เคส)

**Self-update (หา skill ใหม่):** `python -m aim discover` — ค้น GitHub หา skill ที่เพิ่งอัปเดต → LLM ตัดสิน **คุณภาพ+ความใหม่ (ไม่ใช่ดาว)** → เสนอเป็นคิวใน `data/discovered.json` → รีวิวแล้ว `--merge` เข้า catalog → re-index (ต้องมี OPENROUTER_API_KEY; ใส่ GITHUB_TOKEN เพิ่ม rate limit)
